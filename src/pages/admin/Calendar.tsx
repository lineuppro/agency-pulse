...
export default function AdminCalendar() {
  ...
  const handleSaveContent = (data: { id?: string; client_id: string; title: string; description?: string; content_type: ContentType; scheduled_date: string; status?: ContentStatus }) => {
    if (data.id) {
      updateContent.mutate({ id: data.id, title: data.title, description: data.description, content_type: data.content_type, scheduled_date: data.scheduled_date, status: data.status }, { onSuccess: () => { setIsModalOpen(false); setEditingContent(null); } });
    } else {
      createContent.mutate({ client_id: data.client_id, title: data.title, description: data.description, content_type: data.content_type, scheduled_date: data.scheduled_date, status: data.status }, { onSuccess: () => { setIsModalOpen(false); } });
    }
  };

  const handleSidebarSave = (data: { id: string; title: string; description?: string; content_type: ContentType; scheduled_date: string; status: ContentStatus }) => {
    updateContent.mutate({ id: data.id, title: data.title, description: data.description, content_type: data.content_type, scheduled_date: data.scheduled_date, status: data.status }, { onSuccess: () => { setIsSidebarOpen(false); setViewingContent(null); } });
  };
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Gerencie os conteúdos programados</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          {campaigns.length > 0 && (
            <Select value={selectedCampaignId} onValueChange={setSelectedCampaignId}>
              <SelectTrigger className="w-[200px]"><Filter className="h-4 w-4 mr-2" /><SelectValue placeholder="Filtrar campanha" /></SelectTrigger>
              <SelectContent>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={() => { setEditingContent(null); setIsModalOpen(true); }}><Plus className="h-4 w-4 mr-2" />Novo Conteúdo</Button>
        </div>
      </div>

      <div className="flex-1 min-h-0">
        {displayMode === 'calendar' ? (
          <CalendarView contents={contents} view={view} onViewChange={setView} currentDate={currentDate} onDateChange={setCurrentDate} isAdmin={true} onContentClick={handleContentClick} onContentEdit={handleEditContent} onContentDelete={handleDeleteContent} onStatusChange={handleStatusChange} />
        ) : (
          <ContentListView contents={contents} isAdmin={true} />
        )}
      </div>
